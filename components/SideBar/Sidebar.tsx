...
export default function Sidebar({icon,text,active,alert}: SidebarProps) {
      ...
      <aside className="h-screen">
        <nav className="h-full flex flex-col bg-white border-r shadow-sm">
          <div className="p-4 pb-2 flex justify-between items-center">
            <Image alt="" src={logo} className={`overflow-hidden transition-all ${expanded ? "w-32" : "w-0"}`} />
            <button onClick={() => setExpanded((curr) => !curr)} className="p-2 rounded-lg bg-gray-50 hover:bg-gray-100">             
              {expanded ? <ChevronLeftIcon className=""/> : <ChevronRightIcon className=""/>}
            </button>
          </div>
